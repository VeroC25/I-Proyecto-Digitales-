...
loop:
    ...
    #bolita
    jal move_ball       # primero actualiza y guarda la posición vieja
    jal clear_ball      # borra la posición antigua
    jal check_collision
    jal draw_ball
...
la t0, SCORE_RIGHT     # dirección del puntaje derecho
lw t1, 0(t0)           # t1 = valor del puntaje
li t2, 0               # contador de columnas
li t3, 0xf0000000      # dirección base matriz LED
...
move_ball:
    ...
    # Guardar posición ACTUAL antes de mover, como "anterior"
    la t0, BALL_X
    lw t1, 0(t0)
    la t2, BALL_OLD_X
    sw t1, 0(t2)
...
check_collision:
    # Cargar posición de la bola
    la t0, BALL_X
    lw t1, 0(t0)      # t1 = BALL_X
    la t0, BALL_Y
    lw t2, 0(t0)      # t2 = BALL_Y

    # Revisar límites verticales (techo y suelo)
    li t3, 0
    beq t2, t3, invert_dy_2       # Si toca el techo
    li t3, 54
    beq t2, t3, invert_dy_2       # Si toca el suelo

    # Revisar colisión con paleta derecha (X=54)
    la t0, P2_X
    lw t3, 0(t0)                # t3 = P2_X (54)
    la t0, BALL_DX
    lw t4, 0(t0)                # t4 = dirección X de la bola
    blez t4, check_left_paddle  # Solo verificar si se mueve hacia la derecha (DX > 0)

    addi t5, t3, -1
    bne t1, t5, check_left_paddle  # La bola está en X=53 (justo antes de la paleta)
    ...
    blt t2, t3, reset_ball  # Bola arriba de la paleta
    bgt t2, t4, reset_ball  # Bola abajo de la paleta



    j invert_dx                 # Colisión válida, invertir dirección
    ret
check_left_paddle:
    # Revisar colisión con paleta izquierda (X=0)
    la t0, P1_X
    ...
    addi t5, t3, 1
    bne t1, t5, end_collision   # La bola está en X=1 (justo antes de la paleta)
...
reset_ball:
    ...
    # Suponiendo que x está en t0
    li t1, 0          # borde izquierdo
    li t2, 54         # borde derecho

    blt t0, t1, player_right_scored   # x < 0 => punto para jugador derecho
    bgt t0, t2, player_left_scored    # x > 54 => punto para jugador izquierdo
    ret
