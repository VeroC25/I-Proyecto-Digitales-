...
#bolita
BALL_X:     .word 0x1A      # posición inicial X
BALL_Y:     .word 0x1A      # posición inicial Y
BALL_DX:    .word 0x1       # dirección en X (1 = derecha, -1 = izquierda)
BALL_DY:    .word 0x1       # dirección en Y (1 = abajo, -1 = arriba)
...
move_ball:
    ...
    la t2, BALL_DX
    lw t3, 0(t2)     # t3 = BALL_DX
    add t1, t1, t3   # BALL_X += BALL_DX
    sw t1, 0(t0)

    la t0, BALL_Y
    lw t1, 0(t0)     # t1 = BALL_Y
    la t2, BALL_DY
    lw t3, 0(t2)     # t3 = BALL_DY
    add t1, t1, t3   # BALL_Y += BALL_DY
    sw t1, 0(t0)

    # Rebote en bordes superior (0) e inferior (0x36)
    li t4, 0
    li t5, 0x36
    blt t1, t4, invert_dy
    bgt t1, t5, invert_dy
